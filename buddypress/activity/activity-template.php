...
class BP_Activity_Template {
	...
	function __construct( $args ) {
		...
		if ( !is_array( $args ) || func_num_args() > 1 ) {
			_deprecated_argument( __METHOD__, '1.6', sprintf( esc_html__( 'Arguments passed to %1$s should be in an associative array. See the inline documentation at %2$s for more details.', 'blackfyre' ), __METHOD__, __FILE__ ) );
		...
		if ( (int) $this->total_activity_count && (int) $this->pag_num ) {
			$this->pag_links = paginate_links( array(
				...
				'prev_text' => _x( '&larr;', 'Activity pagination previous text', 'blackfyre' ),
				'next_text' => _x( '&rarr;', 'Activity pagination next text', 'blackfyre' ),
				'mid_size'  => 1,
				'add_args'  => array(),
			) );
		}
...
function bp_activity_pagination_count() {
	...
	function bp_get_activity_pagination_count() {
		...
		return sprintf( _n( 'Viewing 1 item', 'Viewing %1$s - %2$s of %3$s items', $total, 'blackfyre' ), $from_num, $to_num, $total );
	}
...
function bp_activity_avatar( $args = '' ) {
	...
	function bp_get_activity_avatar( $args = '' ) {
		...
		$alt_default = !empty( $dn_default ) ? sprintf( esc_html__( 'Profile picture of %s', 'blackfyre' ), $dn_default ) : esc_html__( 'Profile picture', 'blackfyre' );
	...
	function bp_get_activity_secondary_avatar( $args = '' ) {
		...
		switch ( $activities_template->activity->component ) {
			case 'groups' :
				...
				if ( empty( $alt ) ) {
					$alt = esc_html__( 'Group logo', 'blackfyre' );

					if ( ! empty( $name ) ) {
						$alt = sprintf( esc_html__( 'Group logo of %s', 'blackfyre' ), $name );
					}
				}
			...
			case 'blogs' :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( esc_html__( 'Profile picture of the author of the site %s', 'blackfyre' ), get_blog_option( $item_id, 'blogname' ) );
				}
			...
			case 'friends' :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( esc_html__( 'Profile picture of %s', 'blackfyre' ), bp_core_get_user_displayname( $activities_template->activity->secondary_item_id ) );
				}
			...
			default :
				...
				if ( empty( $alt ) ) {
					$alt = sprintf( esc_html__( 'Profile picture of %s', 'blackfyre' ), $activities_template->activity->display_name );
				}
...
function bp_insert_activity_meta( $content ) {
	...
	if ( !bp_is_single_activity() ) {
		 ...
		 */
		$content = apply_filters_ref_array( 'bp_activity_permalink', array( sprintf( '%1$s <a href="%2$s" class="view activity-time-since" title="%3$s">%4$s</a>', $content, bp_activity_get_permalink( $activities_template->activity->id, $activities_template->activity ), esc_html__( 'View Discussion', 'blackfyre' ), $time_since ), &$activities_template->activity ) );
	} else {
...
function bp_activity_comment_count() {
	...
	function bp_activity_get_comment_count( $deprecated = null ) {
		...
		if ( ! empty( $deprecated ) ) {
			_deprecated_argument( __FUNCTION__, '1.2', sprintf( esc_html__( '%1$s no longer accepts arguments. See the inline documentation at %2$s for more details.', 'blackfyre' ), __FUNCTION__, __FILE__ ) );
		}
...
function bp_activity_delete_link() {
	...
	function bp_get_activity_delete_link() {
		...
		$link = '<a href="' . esc_url( $url ) . '" class="button item-button bp-secondary-action ' . $class . ' confirm" rel="nofollow">' . esc_html__( 'Delete', 'blackfyre' ) . '</a>';
...
function bp_activity_filter_links( $args = false ) {
	...
	function bp_get_activity_filter_links( $args = false ) {
		...
		if ( isset( $_GET['afilter'] ) )
			$component_links[] = '<' . $tag . ' id="afilter-clear"><a href="' . esc_url( $link ) . '">' . esc_html__( 'Clear Filter', 'blackfyre' ) . '</a></' . $tag . '>';
...
function bp_mentioned_user_display_name( $user_id_or_username = false ) {
	...
	function bp_get_mentioned_user_display_name( $user_id_or_username = false ) {
		...
		if ( empty( $name ) ) {
			$name = esc_html__( 'a user', 'blackfyre' );
		}
...
function bp_send_public_message_button( $args = '' ) {
	...
	function bp_get_send_public_message_button( $args = '' ) {
		...
		$r = bp_parse_args( $args, array(
			...
			'link_title'        => esc_html__( 'Send a public message on your activity stream.', 'blackfyre' ),
			'link_text'         => esc_html__( 'Public Message', 'blackfyre' ),
...
?>
	...
	<link rel="alternate" type="application/rss+xml" title="<?php bloginfo( 'name' ) ?> | <?php esc_html_e( 'Site Wide Activity RSS Feed', 'blackfyre' ) ?>" href="<?php bp_sitewide_activity_feed_link() ?>" />
